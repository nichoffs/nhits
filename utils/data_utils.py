...
def animate_play_court(
    data, play_id, save_as_video=False, video_filename="animation.html"
):
        ...
        frame = go.Frame(
            data=[
                go.Scatter(
                    ...
                    marker=dict(
                        size=8, color=snapshot[snapshot["player_id"] != -1]["team_id"]
                    ),  # Color by team ID
                    name="Players",
                    textposition="bottom center",
                ),
                go.Scatter(
                    x=snapshot[snapshot["player_id"] == -1][
                        "x"
                    ],  # Include only the ball
                    y=snapshot[snapshot["player_id"] == -1]["y"],
                    mode="markers",
    ...
    fig.update_xaxes(
        ...
        row=1,
        col=1,
    )  # Approx dimensions of a basketball court
    fig.update_yaxes(
        range=[0, 50],
        showgrid=False,
        zeroline=False,
