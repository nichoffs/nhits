...
def prepare_data(df, column, lookback_horizon, forecast_horizon, batch_size):
    ...
    train_data = TensorDataset(torch.cat([x[0] for x in train_sequences]).view(len(train_sequences), lookback_horizon), 
                               torch.cat([x[1] for x in train_sequences]).view(len(train_sequences), forecast_horizon))
    val_data = TensorDataset(torch.cat([x[0] for x in val_sequences]).view(len(val_sequences), lookback_horizon), 
                             torch.cat([x[1] for x in val_sequences]).view(len(val_sequences), forecast_horizon))
...
for feature in features:
    # Prepare the data
    train_dataloader, val_dataloader, scaler = prepare_data(df, feature, lookback_horizon, forecast_size, batch_size)

    # Instantiate the model
    model = NHiTS(lookback_horizon, num_nhits_blocks, input_size, mlp_layer_num, hidden_size, bc_coefs_size, fc_coefs_size, pooling_kernel_size, forecast_size, pooling_mode='max', dropout_prob=0.0)
