...
class NHiTS_Block(nn.Module):
    def __init__(self, lookback_horizon, mlp_layer_num, hidden_size, bc_coefs_size, fc_coefs_size, pooling_kernel_size, forecast_size, forecaster_type='linear', pooling_mode='max', dropout_prob=0.0):
        super().__init__()

        assert forecaster_type in ['fourier', 'linear'], "forecaster must be one of 'fourier', 'linear'"
        assert pooling_mode in ['max', 'avg'], "pooling_mode must be max or avg"

        pooling = nn.MaxPool1d if pooling_mode == 'max' else nn.AvgPool1d
        self.pooling_kernel = pooling(kernel_size=pooling_kernel_size, stride=pooling_kernel_size)

        self.mlps = nn.ModuleList([MLP(hidden_size) for _ in range(mlp_layer_num)])

        self.encode_layer = nn.Linear(lookback_horizon // pooling_kernel_size, hidden_size, bias=False)
