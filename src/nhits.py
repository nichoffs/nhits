...
parser.add_argument('--lookback_horizon', type=int, default=config.get('lookback_horizon'))
...
parser.add_argument('--num_nhits_blocks', type=int, default=config.get('num_nhits_blocks'))
...
parser.add_argument('--pooling_kernel_size', nargs='+', type=int, default=config.get('pooling_kernel_size'))
parser.add_argument('--downsampling_ratios', nargs='+', type=int, default=config.get('downsampling_ratios'))
...
bc_coefs_size = [args.lookback_horizon // downsampling_ratio for downsampling_ratio in args.downsampling_ratios]
fc_coefs_size = [args.forecast_size // downsampling_ratio for downsampling_ratio in args.downsampling_ratios]
...
class NHiTS(nn.Module):
    ...
    def __init__(self, lookback_horizon=120, num_nhits_blocks=3, mlp_layer_num=2, hidden_size=512, pooling_kernel_size=[2,2,2], forecast_size=24, pooling_mode='max', dropout_prob=0.0,num_samples=1000, downsampling_ratios=[24,12,1], batch_size=128):
        super().__init__()
        
        bc_coefs_size = [lookback_horizon // downsampling_ratio for downsampling_ratio in downsampling_ratios]
        fc_coefs_size = [forecast_size // downsampling_ratio for downsampling_ratio in downsampling_ratios]
        
        self.lookback_horizon = lookback_horizon
        self.blocks = nn.ModuleList([NHiTS_Block(lookback_horizon, mlp_layer_num, hidden_size, bc_coefs_size[i], fc_coefs_size[i], pooling_kernel_size[i], forecast_size,forecaster_type='linear', pooling_mode='max', dropout_prob=0.0) for i in range(num_nhits_blocks)])
